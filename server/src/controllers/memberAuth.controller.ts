...
import * as service from '../services/memberAuth.service';
import {
  loginSchema,
  requestOtpSchema,
  verifyOtpSchema,
} from '../validations/memberAuth.validation';
...
export async function requestOtp(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const { error, value } = requestOtpSchema.validate(req.body, { abortEarly: false });
    if (error) throw new ValidationError(error.message);
...
export async function verifyOtpAndChangePassword(
  ...
  try {
    ...
    await service.verifyOtpAndChangePassword(value.email, value.otp, value.newPassword);
...
export async function getDashboard(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    const result = await service.getDashboard(memberId, cooperativeId);
    res.status(200).json({ success: true, data: result });
  } catch (err) {
    next(err);
  }
