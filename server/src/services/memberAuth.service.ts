...
export async function login(email: string, password: string) {
  ...
  if (!member || member.IsActive !== 1) {
    logger.warn(
      { email },
      "Service: member login — member not found or inactive",
    );
...
export async function requestOtp(email: string): Promise<void> {
  ...
  if (!member || member.IsActive !== 1) {
    logger.warn(
      { email },
      "Service: member requestOtp — member not found or inactive",
    );
...
export async function verifyOtpAndChangePassword(
  ...
  if (!stored || Date.now() > stored.expiresAt) {
    logger.warn(
      { email },
      "Service: member verifyOtpAndChangePassword — OTP not found or expired",
    );
  ...
  if (stored.otp !== otp) {
    logger.warn(
      { email },
      "Service: member verifyOtpAndChangePassword — OTP mismatch",
    );
    throw new AppError(
      "OTP is invalid or has expired",
      400,
  ...
  logger.info(
    { memberId: member.Id },
    "Service: member verifyOtpAndChangePassword — password updated",
  );
}
...
export async function getDashboard(memberId: string, cooperativeId: string) {
  ...
  const activeCoopRow = cooperativeRows.find(
    (r) => r.CooperativeId === cooperativeId,
  );

  logger.info(
    { memberId, cooperativeId },
    "Service: member getDashboard — success",
  );
  ...
  return {
    ...
    activeCooperative: {
      ...
      memberSince: activeCoopRow?.DateCreated ?? null,
    },
    cooperatives,
  };
}
