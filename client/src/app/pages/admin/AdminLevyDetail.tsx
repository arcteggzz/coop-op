...
import { getCooperativeMembers } from "../../api/adminCooperatives.api";
import { formatDateTime } from "../../utils/formatDate";
import { useCooperative } from "@/app/hooks/useAdminCooperatives";

const THEME = "#dc2626";
const THEME_GRADIENT = "from-[#dc2626] to-[#b91c1c]";
...
function AssignLevyModal({
  ...
  const { data: membersData, isLoading: loadingMembers } = useQuery({
    queryKey: ["assign-levy-members-admin", cooperativeId],
    queryFn: () =>
      getCooperativeMembers(cooperativeId, { page: 1, pageSize: 1000 }),
  });
            ...
            <div className="p-6">
              <div className="relative mb-3">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 border border-[#e5e7eb] rounded-[10px] text-[13px] focus:outline-none focus:border-[#dc2626] font-['Albert_Sans',sans-serif]"
                />
              </div>
            ...
            <div className="p-6">
              <div className="max-h-48 overflow-y-auto border border-[#e5e7eb] rounded-[10px]">
                {selectedMembers.map((m) => (
                  ...
                  >
                    <div className="size-7 rounded-full bg-[rgba(220,38,38,0.1)] flex items-center justify-center text-[#dc2626] text-[11px] font-semibold shrink-0">
                      {m.firstName[0]}
                      {m.lastName[0]}
                    </div>
                    <div className="min-w-0">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={waiveMutation.isPending}
              className="flex-1 cursor-pointer py-2.5 rounded-[10px] text-[14px] font-semibold text-white bg-[#dc2626] hover:bg-[#b91c1c] transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
            >
...
export default function AdminLevyDetail({
  embedded = false,
}: {
  embedded?: boolean;
}) {
  const { cooperativeId, levyId } = useParams<{
    cooperativeId: string;
    levyId: string;
  }>();
  ...
  const handleDelete = () => {
    ...
    deleteMutation.mutate(levyId, {
      onSuccess: () => navigate(`/admin/cooperatives/${cooperativeId}`),
    });
  };
  ...
  if (!levy) {
    ...
    if (embedded) return notFound;
    return <AdminLayout>{notFound}</AdminLayout>;
  }

  const content = (
    <>
      <div className="font-['Albert_Sans',sans-serif] flex flex-col h-full">
        {/* ── White top bar ────────────────────────────────────────────── */}
        <div className="px-8 pt-8 pb-6 border-b border-[#e5e7eb] bg-white">
          ...
          <div className="flex items-center gap-2 mb-3 text-[13px] text-[#6b7280]">
            ...
            <Link
              ...
              className="hover:text-[#dc2626] transition-colors"
            >
              {levyLoading ? "..." : (coop?.name ?? "Cooperative")}
            </Link>
            <span>/</span>
            <Link
              to={`/admin/cooperatives/${cooperativeId}/levies`}
              className="hover:text-[#dc2626] transition-colors"
            >
              Levies
            </Link>
            ...
            <span className="text-[#101828] font-medium">
              {levyLoading ? "..." : (levy?.name ?? "Levy Detail")}
            </span>
          </div>
        ...
        <div className="flex-1 p-8 bg-[#fafbfd] overflow-auto">
          ...
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={() =>
                navigate(`/admin/cooperatives/${cooperativeId}/levies`)
              }
