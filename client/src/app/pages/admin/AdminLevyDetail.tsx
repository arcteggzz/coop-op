...
function AssignLevyModal({
  ...
  const { data: membersData, isLoading: loadingMembers } = useQuery({
    ...
    queryFn: () => getCooperativeMembers(cooperativeId, { page: 1, pageSize: 1000 }),
  });
...
export default function AdminLevyDetail({ embedded = false }: { embedded?: boolean }) {
  ...
  const {
    data: levyData,
    isLoading: levyLoading,
  } = useLevy(baseUrl, cooperativeId!, levyId!);
  ...
  const content = (
    ...
    <div className="font-['Albert_Sans',sans-serif] px-8 pt-8 pb-10">
        {/* Back */}
        <button
          onClick={() => navigate(`/admin/cooperatives/${cooperativeId}`)}
