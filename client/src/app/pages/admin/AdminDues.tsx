...
function CreateDueModal({
  ...
  const { data: wallets, isLoading: walletsLoading } = useAdminCooperativeWallets(cooperativeId);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-8 font-['Albert_Sans',sans-serif]">
        ...
        <p className="text-[14px] text-[#6b7280] mb-6">
          Are you sure you want to delete <span className="font-semibold text-[#374151]">{scheduleName}</span>? This cannot be undone.
        </p>
...
// ─── Main AdminDues Component ─────────────────────────────────────────────────

export default function AdminDues({ cooperativeId }: { cooperativeId: string }) {
  ...
  const { data: schedulesData, isLoading: schedulesLoading } = useDueSchedules(
    baseUrl,
    cooperativeId,
  );
              ...
              {schedulesLoading ? (
                ...
                <tr>
                  <td colSpan={9} className="px-5 py-10 text-center text-[14px] text-[#9ca3af]">
                    No due schedules yet. Create one to get started.
                  </td>
                </tr>
              ) : (
                schedules.map((s, idx) => (
                  <tr key={s.id} className="border-b border-[#f3f4f6] hover:bg-[#fafafa] transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <Link
                        to={`/admin/cooperatives/${cooperativeId}/dues/${s.id}`}
